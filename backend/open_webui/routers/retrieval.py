...
async def update_reranking_config(request: Request, form_data: RerankingModelUpdateForm, user=Depends(get_admin_user)):
    log.info(f"Updating reranking model: {request.app.state.config.RAG_RERANKING_MODEL} to {form_data.reranking_model}")
    try:
...
async def update_rag_config(request: Request, form_data: ConfigUpdateForm, user=Depends(get_admin_user)):
    ...
    if form_data.content_extraction is not None:
        ...
        request.app.state.config.CONTENT_EXTRACTION_ENGINE = form_data.content_extraction.engine
        request.app.state.config.TIKA_SERVER_URL = form_data.content_extraction.tika_server_url
    ...
    if form_data.web is not None:
        ...
        request.app.state.config.GOOGLE_PSE_ENGINE_ID = form_data.web.search.google_pse_engine_id
        request.app.state.config.BRAVE_SEARCH_API_KEY = form_data.web.search.brave_search_api_key
        request.app.state.config.KAGI_SEARCH_API_KEY = form_data.web.search.kagi_search_api_key
        request.app.state.config.MOJEEK_SEARCH_API_KEY = form_data.web.search.mojeek_search_api_key
        request.app.state.config.BOCHA_SEARCH_API_KEY = form_data.web.search.bocha_search_api_key
        request.app.state.config.SERPSTACK_API_KEY = form_data.web.search.serpstack_api_key
        ...
        request.app.state.config.SEARCHAPI_API_KEY = form_data.web.search.searchapi_api_key
        request.app.state.config.SEARCHAPI_ENGINE = form_data.web.search.searchapi_engine
        ...
        request.app.state.config.BING_SEARCH_V7_ENDPOINT = form_data.web.search.bing_search_v7_endpoint
        request.app.state.config.BING_SEARCH_V7_SUBSCRIPTION_KEY = form_data.web.search.bing_search_v7_subscription_key
        ...
        request.app.state.config.RAG_WEB_SEARCH_RESULT_COUNT = form_data.web.search.result_count
        request.app.state.config.RAG_WEB_SEARCH_CONCURRENT_REQUESTS = form_data.web.search.concurrent_requests
        request.app.state.config.RAG_WEB_SEARCH_TRUST_ENV = form_data.web.search.trust_env
        request.app.state.config.RAG_WEB_SEARCH_DOMAIN_FILTER_LIST = form_data.web.search.domain_filter_list
...
async def update_query_settings(request: Request, form_data: QuerySettingsForm, user=Depends(get_admin_user)):
    ...
    request.app.state.config.ENABLE_RAG_HYBRID_SEARCH = form_data.hybrid if form_data.hybrid else False
...
async def process_web_search(request: Request, form_data: SearchForm, user=Depends(get_verified_user)):
    try:
        logging.info(f"trying to web search with {request.app.state.config.RAG_WEB_SEARCH_ENGINE, form_data.query}")
        web_results = search_web(request, request.app.state.config.RAG_WEB_SEARCH_ENGINE, form_data.query)
    except Exception as e:
        log.exception(e)
